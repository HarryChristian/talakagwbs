...
<nav class="navbar navbar-default" style="border: 5px #0985c5; background-color: #0985c5;">
  ...
  <li class="dropdown">
    <a href="javascript:void(0)" class="dropbtn" style="color: white; background-color: #46b1e4; font-size: 14px;">Clients<span class="caret"></span></a>
    <div class="dropdown-content">
      <a href="rclients.php" style="color: white;">Residential</a>
      <a href="cclients.php" style="color: white; background-color: #46b1e4;">Commercial</a>
    </div>
  </li>
...
<center><strong style="font-size: 30px;">Commercial Clients</strong></center>
</div>

<div class="container" style="width:1280px;">
<form action="multcomclient.php" method="post">
<a class="btn btn-primary" rel="facebox" href="addcomclient.php" style="margin-bottom: 10px;">Add New Client</a>
<button class="btn btn-success" style="margin-right: 0px; margin-bottom: 10px; margin-left: 800px;" name="edit" type="submit" id="validate">Edit Selected</button>
<button class="btn btn-warning" style="margin-right: 0px; margin-bottom: 10px;" name="delete" type="submit" id="validate" onclick="return confirm('Delete Selected Client/s? Are You Sure?');">Delete Selected</button>
<input type="checkbox" name="select-all" id="select-all" style="margin-left: 10px;"> <strong>All</strong>
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="table table-hover table-bordered" id="example">
    ...
    <tbody>
    <?php 
    $query=mysqli_query($con, "SELECT * FROM commercial_owners ORDER BY account ASC")or die(mysqli_error());
    while($row=mysqli_fetch_array($query)){ 
    $id=$row['id'];
    ?>
      <tr>        
        <td style="text-align:center;"><?php echo $row['account'] ?></td>
        <td style="text-align:center;"><?php echo $row['barangay'] ?></td>
        <td style="text-align:center;"><?php echo $row['meterno'] ?></td>

        <?php
 echo "<td style='width: 12%; text-align:center;'><a class='btn btn-success' rel='facebox' href='editcom.php?id=".$row['id']."'>Edit</a> ";
 echo "<a class='btn btn-warning' rel='facebox' href='delcom.php?id=".$row['id']."'>Delete</a></td>";
      ?>
